...
hbwipbrt <- rast("UplandWetlandGradient/data/derived_data/hbwip_hydrog_strata_brt.tif")
...
hb_pedons_ext |> group_by(hpu) |> reframe(result = mean(WET, na.rm = T)) |>
    ggplot(aes(x = reorder(hpu, desc(result), FUN = "mean"))) +
        geom_col(aes(y = result)) 

hb_pedonsbrt_ext |> group_by(hpu) |> reframe(result = mean(WET, na.rm = T)) |>
    ggplot(aes(x = reorder(hpu, desc(result), FUN = "mean"))) +
        geom_col(aes(y = result)) 
```
...
```{r}
ggplot(w3_wells, aes(x = date_hour)) +
    geom_point(aes(y = mean_levelJD01), colour = "rosybrown4", alpha = 0.3) + 
    geom_point(aes(y = mean_levelJD02), colour = "rosybrown3", alpha = 0.3) + 
    geom_point(aes(y = mean_levelJD03), colour = "rosybrown1", alpha = 0.3)
```
